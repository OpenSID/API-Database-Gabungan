...
class DataPresisiJaminanSosial extends BaseUuidModel
{
    use HasFactory, FilterWilayahTrait;

    protected $guarded = [];

    protected $table = 'data_presisi_jaminan_sosial';
    ...
    protected function scopeKepalaRtm($query)
    {
        return $query->join('tweb_rtm', function ($join) {
            $join->on('tweb_rtm.id', '=', 'data_presisi_jaminan_sosial.rtm_id')
                ->on('tweb_rtm.nik_kepala', '=', 'data_presisi_jaminan_sosial.anggota_id');
        });
    }
