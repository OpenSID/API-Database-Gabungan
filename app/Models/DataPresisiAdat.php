...
class DataPresisiAdat extends BaseUuidModel
{
    use HasFactory, FilterWilayahTrait;

    protected $guarded = [];

    protected $table = 'data_presisi_aktivitas_adat';
    ...
    {
        return $this->hasMany(DataPresisiAdat::class, 'rtm_id', 'rtm_id')->where('data_presisi_tahun_id', '!=', $this->data_presisi_tahun_id);
    }
    ...
    protected function scopeKepalaRtm($query)
    {
        return $query->join('tweb_rtm', function ($join) {
            $join->on('tweb_rtm.id', '=', 'data_presisi_aktivitas_adat.rtm_id')
                ->on('tweb_rtm.nik_kepala', '=', 'data_presisi_aktivitas_adat.anggota_id');
        });
    }
